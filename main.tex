...
Under \texttt{Default}, the median cold first-query latencies are 503~\textmu s for Scattered-Zipf-100K, 733~\textmu s for Uniform-100K, and 1072~\textmu s for Tail-Mixed, against 1 to 3 \textmu s with a warm cache. These cold values are the denominators for every improvement reported in Section~\ref{sec:evaluation}. The workload traces are published with the artifact, checksummed, together with the generator that drives the 10-seed analysis of Section~\ref{sec:stats}.
