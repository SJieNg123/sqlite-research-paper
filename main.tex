...
\textbf{A frugal-coverage design rule (Sections~\ref{sec:strategy} and~\ref{sec:evaluation})}.  Charging preprocessing reverses the naive ranking. The full working-set cache dump minimizes first-query latency but regresses end-to-end cold starts by roughly an order of magnitude on broad workloads, because delivery cost dominates. Delivering the same set through window-chunked hints rather than per-page ones cuts that cost by about $3.6\times$ and is the stronger form of the baseline, and the dump still regresses on those workloads. The robust operating point is a set under 2 MB that covers the B+tree interior navigation path, which yields a 25\% to 30\% warm-process end-to-end reduction across skewed and uniform read workloads. Hot-leaf prefetching is a conditional add-on that helps only under an independently verified hotspot, and at matched footprint a frequency-ranked dump is statistically indistinguishable from the type-aware selector. Page-type knowledge therefore serves as an interpretable coverage guarantee that stays stable when the read hotspot moves, rather than as a uniquely faster selector. A physical layout rewriter that clusters interior pages at the file head is a negative result under our workloads.
...
We first report first-query latency alone, before charging preprocessing in Section~\ref{sec:eval-tradeoff}. Against the cold baselines of Section~\ref{sec:workloads} (492, 734, and 1013 \textmu s), Dump reaches the lowest first-query latency on every workload: 96 \textmu s on Scattered-Zipf-100K ($-80\%$), 97 \textmu s on Uniform-100K ($-87\%$), and 91 \textmu s on Tail-Mixed ($-91\%$). Judged by this metric alone, Dump wins outright. The targeted strategies reach $-23\%$ to $-83\%$, with a ceiling that depends on the workload (Table~\ref{tab:ceiling} and Figure~\ref{fig:firstq-bars}).

The ceiling has a structural explanation. At first-query time every page is cold, and interior prefetching removes the interior faults but leaves the first leaf fault. On Scattered-Zipf-100K the interior skeleton alone captures most of the benefit ($-27\%$ to $-33\%$), and adding leaves yields diminishing returns: \texttt{Skel+500} reaches $-63\%$ and Dump $-80\%$. On Uniform-100K there is no hot set, every query hits a cold leaf, and interior-only prefetching is capped at $-45\%$, with only the full dump going further, to $-87\%$. On Tail-Mixed the interior-only ceiling is $-36\%$, but \texttt{Skel+10} reaches $-83\%$ because about half the queries are not-found probes that descend to the same rightmost leaf, which \texttt{Skel+10} includes. This is a property of the key range rather than of tail reads in general, as Section~\ref{sec:eval-attribution} confirms with a pure-hit control.
...
\midrule
Scattered-Zipf-100K & $-33\%$ & \texttt{Skel+500}: $-63\%$, \texttt{Dump}: $-80\%$ \\
Uniform-100K        & $-45\%$ & \texttt{Dump}: $-87\%$ \\
Tail-Mixed          & $-36\%$ & \texttt{Skel+10}: $-83\%$, \texttt{Dump}: $-91\%$ \\
\bottomrule
\end{tabular}
\label{tab:ceiling}
\end{table}
...
\texttt{Dump} & $\sim$220 \textmu s & 0.2--7 ms & deliver \\
\bottomrule
\end{tabular}
\label{tab:overhead}
\end{table}
...
\multicolumn{2}{c}{\shortstack{\textbf{Scattered-Zipf-100K}\\(baseline 492\,\textmu s)}} & 
\multicolumn{2}{c}{\shortstack{\textbf{Tail-Mixed}\\(baseline 1013\,\textmu s)}} & 
\multirow{2}{*}{\textbf{Cross-seed verdict}} \\
\cmidrule(lr){2-3} \cmidrule(lr){4-5}
& $\text{e2e}_{\text{ext}}$ & $\text{e2e}_{\text{warm}}$ & $\text{e2e}_{\text{ext}}$ & $\text{e2e}_{\text{warm}}$ & \\
\midrule
Baseline        & 492            & 492            & 1013          & 1013          & n/a \\
\texttt{Skel-5} & 644 (+31\%)   & 451 ($-$8\%)   & 1326 (+31\%)  & 1099 (+8\%)   & Scattered-Zipf-100K: \textit{tie}, Tail-Mixed: \textit{robust} (worse) \\
\texttt{Skel}   & 647 (+32\%)   & 414 ($-$16\%)  & 905 ($-$11\%) & 719 ($-$29\%) & Scattered-Zipf-100K: \textit{robust} ($-$27\%), Tail-Mixed: \textit{robust} ($-$37\%) \\
\texttt{Dump}   & 7306 (+1386\%) & 7076 (+1339\%) & 1030 (+2\%)    & 846 ($-$16\%) & Scattered-Zipf-100K: \textit{robust} (worse), Tail-Mixed: \textit{robust} ($-$9\%) \\
\bottomrule
\end{tabularx}
\label{tab:e2e-ac}
\end{table*}
...
\caption{Warm-process end-to-end decomposition, same batch as Figure~\ref{fig:firstq-bars}. Each bar stacks first query and delivery, so its height is \texttt{e2e\_warm}, and each strategy is drawn twice, once per delivery mechanism. Light bars issue one hint per page and dark bars one hint per 32-page readahead window, so hue encodes the component and lightness the mechanism. The pair shows that window chunking leaves first query untouched and cuts only delivery, and that the saving scales with how much a strategy had to deliver. \texttt{Dump} falls from $+1339\%$ to $+311\%$ on Scattered-Zipf-100K and from $+863\%$ to $+178\%$ on Uniform-100K, and on Tail-Mixed from $-16\%$ to $-69\%$, whereas the targeted arms move by a few points. \texttt{Skel+500} regresses by $+115\%$ and $+77\%$ under per-page delivery, and \texttt{Skel} sits below baseline on all three workloads. Labels are paired changes at seed 1, whose Scattered-Zipf-100K baseline is an instantiation outlier, so the cross-seed \texttt{Dump} figure is $+770\%$ rather than $+1339\%$. The \texttt{Skel-5} improvements are single-instantiation only (Table~\ref{tab:e2e-ac}).}
...
\textbf{Why the Tail-Mixed result is narrow.} About half of Tail-Mixed's lookups are not-found probes, and every one descends the right edge to the same rightmost leaf, an extreme key-range-induced hotspot. The corrected \texttt{Skel+10} result is accordingly bimodal: about $-72\%$ when the measured first query is itself a not-found probe and about $-35\%$, the interior-skeleton regime, when it is a hit, for a mean of $-57\%$. On the matched pure-hit control Tail-Hit, \texttt{Skel+10} returns to about $-29\%$, statistically matching \texttt{Skel} ($-28\%$) and a footprint-matched frequency dump ($-30\%$). Once the not-found concentration is removed, leaf frequency adds nothing beyond the skeleton. We report the Tail-Mixed number as a scoped, mechanism-specific result.
...
\texttt{leaf\_rand\_K10} & 10 & $-1\%$ $[-3, +1]$ & $+7\%$ (robust, worse) \\
\texttt{leaf\_freq\_K10} & 10 & $-10\%$ $[-21, +1]$ & $-2\%$ (tie) \\
\texttt{Skel+10} & 14 & $-66\%$ $[-77, -54]$ & $-57\%$ (robust, bimodal) \\
\bottomrule
\end{tabular}
\label{tab:ablation}
\end{table}
...
\begin{table*}[htbp]
\centering
\caption{Frequency-ranked partial dump versus targeted prefetch, warm-process end-to-end, 10-seed means with $95\%$ bootstrap CIs. All cells are from the same 10-seed batch as Tables~\ref{tab:e2e-ac},~\ref{tab:ablation} and~\ref{tab:seeds}, so every column is mutually comparable. The upper block issues one hint per page and the lower block one hint per 32-page readahead window (Section~\ref{sec:selection-delivery}). Window chunking leaves selection untouched and changes only what delivery costs, which is what reorders the table: under it the full dump overtakes targeted prefetch on the two tail workloads while still losing by a wide margin on the two broad ones. Tail-Hit is the pure-hit control of Section~\ref{sec:workloads}, included here because the comparison turns on it.}
\begin{tabular}{l l r r r r}
\toprule
\textbf{Arm} & \textbf{Footprint} & \textbf{Scattered-Zipf-100K} & \textbf{Uniform-100K} & \textbf{Tail-Mixed} & \textbf{Tail-Hit} \\
\midrule
\multicolumn{6}{l}{\textit{per-page hint}} \\
\texttt{Skel+10} (targeted) & 14--28 pages & $-38\%$ $[-52, -25]$ & $-25\%$ $[-31, -15]$ & $-57\%$ $[-68, -46]$ & $-29\%$ $[-36, -20]$ \\
\texttt{Dump-14} (ranked) & 14 pages & $-31\%$ $[-42, -22]$ & $-26\%$ $[-33, -15]$ & $-57\%$ $[-68, -46]$ & $-30\%$ $[-37, -22]$ \\
\texttt{Dump-500} (ranked) & 500 pages & $+81\%$ $[+36, +149]$ & $+41\%$ $[+24, +57]$ & $-10\%$ $[-14, -6]$ & $+14\%$ $[-1, +29]$ \\
\texttt{Dump} (full dump) & $\sim$4400 pages & $+770\%$ $[+681, +907]$ & $+724\%$ $[+649, +826]$ & $-9\%$ $[-13, -5]$ & $+72\%$ $[+49, +102]$ \\
\midrule
\multicolumn{6}{l}{\textit{window-chunked hint}} \\
\texttt{Skel+10} (targeted) & 14--28 pages & $-45\%$ $[-59, -32]$ & $-32\%$ $[-38, -24]$ & $-62\%$ $[-75, -49]$ & $-34\%$ $[-40, -26]$ \\
\texttt{Dump-14} (ranked) & 14 pages & $-39\%$ $[-49, -31]$ & $-33\%$ $[-39, -25]$ & $-62\%$ $[-75, -50]$ & $-34\%$ $[-40, -26]$ \\
\texttt{Dump-500} (ranked) & 500 pages & $+29\%$ $[+11, +47]$ & $+22\%$ $[+6, +36]$ & $-66\%$ $[-68, -65]$ & $-24\%$ $[-36, -12]$ \\
\texttt{Dump} (full dump) & $\sim$4400 pages & $+150\%$ $[+125, +189]$ & $+137\%$ $[+115, +168]$ & $-66\%$ $[-68, -65]$ & $-41\%$ $[-49, -31]$ \\
\bottomrule
\end{tabular}
...
\textbf{Workload instantiation.} Table~\ref{tab:seeds} reports cross-seed warm-process improvements from the 10-seed sweep. The robust general result is the interior skeleton: \texttt{Skel} improves warm-process latency by $-27\%$ on Scattered-Zipf-100K, $-26\%$ on Uniform-100K, and $-28\%$ on Tail-Hit, all with CIs excluding zero. The larger figures belong to Scattered-Zipf-100K \texttt{Skel+10} at $-38\%$, a skew bonus, and to \texttt{Tail-Mixed Skel} at $-37\%$, inflated by the not-found hotspot. \texttt{Tail-Mixed Skel+10} is $-57\%$ but bimodal (Section~\ref{sec:eval-attribution}). \texttt{Skel-5} does not survive: tie on Scattered-Zipf-100K ($-5\%$, 6 of 10 seeds) and directional toward regression on Uniform-100K, where 8 of 10 seeds worsen (median $+5\%$) and two strongly improving seeds pull the mean to $-1\%$. Structural prefetching without access observation is unreliable. The sweep also shows why single instantiations mislead: the main batch drew a Scattered-Zipf-100K instance with an unusually cheap first query (492 \textmu s against a cross-seed median of 881 \textmu s), so its \texttt{Skel+10} estimate of $-8\%$ understates the cross-seed $-38\%$, and it overstated Uniform-100K \texttt{Skel-5} at $-35\%$ against a cross-seed $-1\%$.
...
\midrule
Tail-Mixed \texttt{Skel+10} & $-75\%$\textsuperscript{$\dagger$} & $-57\%$ $[-68, -46]$ & \textit{robust, bimodal} \\
Tail-Mixed \texttt{Skel} & $-29\%$ & $-37\%$ $[-40, -34]$ & \textit{robust} (10/10) \\
Scattered-Zipf-100K \texttt{Skel+10} & $-8\%$ & $-38\%$ $[-52, -25]$ & \textit{robust} (10/10) \\
Scattered-Zipf-100K \texttt{Skel} & $-16\%$ & $-27\%$ $[-31, -23]$ & \textit{robust} (10/10) \\
Uniform-100K \texttt{Skel} & $-33\%$ & $-26\%$ $[-33, -16]$ & \textit{robust} (9/10) \\
Uniform-100K \texttt{Skel+10} & $-31\%$ & $-25\%$ $[-31, -15]$ & \textit{robust} (9/10) \\
Scattered-Zipf-100K \texttt{Skel-5} & $-8\%$ & $-5\%$ $[-15, +5]$ & \textit{tie} (6/10) \\
Uniform-100K \texttt{Skel-5} & $-35\%$ & $-1\%$ $[-11, +7]$ & \textit{directional} (8/10 regress) \\
\bottomrule
\end{tabular}
\label{tab:seeds}
\end{table*}
...
Three rules summarize the results. First, for warm-process deployments the default is \texttt{Skel} on the Default layout: it improves end-to-end latency on every workload, survives cross-seed validation, and needs under 2 MB. Second, a small leaf budget (\texttt{Skel+10}) is worth adding only after an independently verified, stable skew, and when the read hotspot moves the durable choice is structural coverage or a periodic plan refresh rather than a frozen frequency plan. Third, the full dump belongs on the cold-start critical path only where the working set is small and delivery is window-chunked. Under per-page delivery its cost dominates on every workload whose working set runs to several megabytes, and even window-chunked it still regresses by $+150\%$ and $+137\%$ on the two 100K-key workloads. Where the working set is small the same mechanism makes it competitive, and on the two tail workloads it is the best arm we measured. The rule is therefore conditional on measuring the working set rather than on the strategy alone, and the exception narrows as the database grows, since at $10\times$ the Tail-Mixed dump returns to a $+29\%$ regression (Section~\ref{sec:eval-robustness}). \texttt{Skel-5} should not be used alone, since its single-workload wins do not survive cross-seed validation. Where only structural selection is available, prefetch the full interior set (\texttt{Skel-92}, 368 KB), which does not depend on offset order.
